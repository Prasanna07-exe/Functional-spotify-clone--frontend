...
buttons.forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    currentButton = btn;
    iconPath.setAttribute('d', pausePath);
    songTitleElem.innerText = title;

    currentAudio.addEventListener('ended', () => {
      iconPath.setAttribute('d', playPath);
      timeElem.innerText = `0:00 / 0:00`;
      songTitleElem.innerText = 'Not playing';
      currentAudio = null;
      currentButton = null;
    });
    ...
    currentAudio.addEventListener('timeupdate', () => {
      if (currentAudio) {
        ...
        timeElem.innerText = `${current} / ${total}`;
      }
    });
    currentAudio.play();
  });
...
plus.addEventListener('click', () => {
  alert("You cant do this action without Signing In / Logging In");
});
...
leftbuttons.forEach(lb => {
  lb.addEventListener('click', () => {
    alert("Please Login/SignIn to continue");
  });
});

const login = document.getElementById("login");
login.addEventListener('click', () => {
  alert("This is just a frontend part *Database unavailable* (Just hear the songs bro!!)");
});
